...
def calculate_confusion_matrix(
    predictions: List[int], ground_truths: List[int], num_classes: int
) -> np.ndarray:
    """
...
class DetectionMetrics:
    ...
    def add_predictions(
        ...
        for box, score, label in sorted(
            zip(pred_boxes, pred_scores, pred_labels), key=lambda x: x[1], reverse=True
        ):
            label_int = int(label)
            gt_indices = [
                index for index, gt_label in enumerate(gt_labels) if int(gt_label) == label_int
            ]
            best_iou = 0.0
            best_gt_idx = -1
            for i in gt_indices:
    ...
    def compute_metrics(self) -> Dict[str, float]:
        ...
        mAP, class_aps = calculate_map(
            self.predictions, self.ground_truths, self.iou_threshold, self.num_classes
        )
