...
class CameraMonitor:
    ...
    def start(self):
        ...
        while self.running:
            ...
            if frame_count:
                ...
                if len(results[0].boxes) > 0:
                    ...
                    print(
                        f"[ALERT] Graffiti detected on {self.camera_id} at {timestamp} "
        ...
        self.cap.release()
        print(
            f"[INFO] Stopped monitoring camera: {self.camera_id} "
...
class MultiCameraSurveillance:
    ...
    def __init__(self, model_path: str, cameras_config: Dict, alert_config: Optional[Dict] = None):
    ...
    def start_monitoring(self):
        ...
        print(f"[INFO] Starting surveillance system with {len(enabled_cameras)} enabled cameras")
...
def main():
    ...
    parser.add_argument(
        "--cameras", type=str, required=True, help="Path to cameras configuration JSON file"
    )
    parser.add_argument(
        "--alert-config", type=str, default=None, help="Path to alert configuration JSON file"
    )
    ...
    surveillance = MultiCameraSurveillance(
        model_path=args.model, cameras_config=cameras_config, alert_config=alert_config
    )
