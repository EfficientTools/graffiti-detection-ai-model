...
def parse_args():
    ...
    parser.add_argument(
        "--output", type=str, default="outputs/predictions", help="Output directory for predictions"
    )
    ...
    parser.add_argument(
        "--hide-conf", action="store_true", help="Hide confidence scores in visualization"
    )
...
def process_image(model, image_path, args, output_dir):
    ...
    if len(boxes) > 0:
        image_rgb = draw_boxes(
            image_rgb, boxes, labels.tolist(), scores, class_names, thickness=args.line_thickness
        )
    ...
    if args.save_txt:
        ...
        with open(txt_path, "w") as f:
            for box, label, score in zip(boxes, labels, scores):
                ...
                if args.save_conf:
                    f.write(
                        f"{label} {x_center:.6f} {y_center:.6f} {width:.6f} {height:.6f} {score:.4f}\n"
                    )
                else:
...
def main():
    ...
    if args.source == "0" or args.source.isdigit():
        ...
        if source_path.suffix.lower() in video_extensions:
            ...
            # Consume streaming results so Ultralytics writes every output frame.
            for _ in results:
                continue
