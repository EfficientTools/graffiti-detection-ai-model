...
class AntiVandalismDashboard:
    ...
    def _draw_header(self, img):
        ...
        cv2.putText(
            img, status_text, (self.width - 120, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.8, status_color, 2
        )
    ...
    def _draw_stats_panel(self, img):
        ...
        # Background
        cv2.rectangle(
            img, (20, y_start), (self.width - 20, y_start + panel_height), (50, 50, 50), -1
        )
        ...
        for i, (label, value, color) in enumerate(stats):
            ...
            # Label
            cv2.putText(
                img, label, (x, y_start + 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200, 200, 200), 1
            )

            # Value
            cv2.putText(img, value, (x, y_start + 90), cv2.FONT_HERSHEY_BOLD, 1.5, color, 2)
    ...
    def _draw_timeline(self, img):
        ...
        if len(self.detection_history) > 0:
            ...
            # Base line
            cv2.line(img, (40, timeline_y), (40 + timeline_width, timeline_y), (100, 100, 100), 2)
    ...
    def _draw_alert_list(self, img):
        ...
        cv2.rectangle(
            img, (20, y_start), (self.width // 2 - 10, y_start + panel_height), (50, 50, 50), -1
        )
        cv2.putText(
            img,
            "RECENT ALERTS",
        ...
        for i, det in enumerate(list(self.detection_history)[-8:]):
            ...
            cv2.putText(
                img, alert_text, (70, y + 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (220, 220, 220), 1
            )
    ...
    def _draw_camera_status(self, img):
        ...
        cv2.rectangle(
            img, (x_start, y_start), (self.width - 20, y_start + panel_height), (50, 50, 50), -1
        )
        cv2.putText(
            img,
            "CAMERA STATUS",
        ...
        for i, (name, status, is_active) in enumerate(cameras):
            ...
            # Camera name
            cv2.putText(
                img, name, (x_start + 50, y + 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (220, 220, 220), 1
            )

            # Status
            cv2.putText(
                img, status, (x_start + 400, y + 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, status_color, 1
            )
    ...
    def run(self, update_interval: float = 1.0):
        """Run the dashboard"""
        print("🖥️  Starting Anti-Vandalism Dashboard...")
...
def main():
    ...
    parser.add_argument(
        "--stats-file", type=str, default="outputs/stats.json", help="Path to statistics JSON file"
    )
    parser.add_argument(
        "--update-interval", type=float, default=1.0, help="Dashboard update interval in seconds"
    )
